...
export class SmsService implements NotificationFactory {
    ...
    async sendSMS(
        ...
        const smsLenzUrl = this.configService.get<string>('SMS_API_URL', "https://api.smslenz.com/v1/sms/send");
        ...
        try {
            await this.httpService.get<SmsLenzResponse>(smsLenzUrl, { params }).toPromise();
        } catch (error) {
            this.logger.error('Error sending SMS:', error);
            throw new AppError('Failed to send SMS', HttpStatus.INTERNAL_SERVER_ERROR);
        }
